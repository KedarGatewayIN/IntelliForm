...
export default function FormElements({
  ...
  const [chatMessages, setChatMessages] = useState([
    ...
    { role: 'assistant', content: 'Hello! I\'m here to help. What can I assist you with today?' }
  ]);
        ...
        return (
          <div className="border border-gray-200 rounded-lg p-4">
            ...
            <div className="flex space-x-2">
              <Input placeholder="Type your message..." disabled className="flex-1" />
  ...
  return (
    <Card 
      ...
      } ${field.type === 'ai_conversation' ? 'border-purple-200 bg-gradient-to-r from-purple-50/30 to-blue-50/30' : ''}`}
      onClick={onSelect}
    >
      <div className="p-4">
        {/* Field Header */}
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-2">
            <Icon className={`h-4 w-4 ${field.type === 'ai_conversation' ? 'text-secondary' : 'text-gray-500'}`} />
          ...
          <div className={`flex items-center space-x-1 ${isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-opacity`}>
            <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); onMove("up"); }}>
              <ArrowUpIcon className="h-3 w-3" />
            </Button>
            <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); onMove("down"); }}>
