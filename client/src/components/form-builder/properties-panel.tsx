                ...
                <>
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={
                        selectedField.validation?.find(rule => rule.type === "min")?.value || ""
                      }
                      onChange={(e) => {
                        const value = e.target.value;
                        const currentValidation = selectedField.validation || [];
                        const existingMinRule = currentValidation.find(rule => rule.type === "min");
                        ...
                        if (value && Number(value) > 0) {
                          if (existingMinRule) {
                            ...
                            const updatedValidation = currentValidation.map(rule =>
                              rule.type === "min" 
                                ? { ...rule, value: Number(value), message: `Minimum ${value} characters required` }
                                : rule
                            );
                          ...
                          const updatedValidation = currentValidation.filter(rule => rule.type !== "min");
                          onUpdateField({ validation: updatedValidation });
                        }
                      }}
                    />
                    {selectedField.validation?.find(rule => rule.type === "min") && (
                      <Input
                        placeholder="Custom error message"
                        value={
                          selectedField.validation?.find(rule => rule.type === "min")?.message || ""
                        }
                        onChange={(e) => {
                          ...
                          const updatedValidation = currentValidation.map(rule =>
                            rule.type === "min" 
                              ? { ...rule, message: message || `Minimum ${rule.value} characters required` }
                              : rule
                          );
                          onUpdateField({ validation: updatedValidation });
                        }}
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={
                        selectedField.validation?.find(rule => rule.type === "max")?.value || ""
                      }
                      onChange={(e) => {
                        const value = e.target.value;
                        const currentValidation = selectedField.validation || [];
                        const existingMaxRule = currentValidation.find(rule => rule.type === "max");
                        ...
                        if (value && Number(value) > 0) {
                          if (existingMaxRule) {
                            ...
                            const updatedValidation = currentValidation.map(rule =>
                              rule.type === "max" 
                                ? { ...rule, value: Number(value), message: `Maximum ${value} characters allowed` }
                                : rule
                            );
                            onUpdateField({ validation: updatedValidation });
                          } else {
                          ...
                          const updatedValidation = currentValidation.filter(rule => rule.type !== "max");
                          onUpdateField({ validation: updatedValidation });
                        }
                      }}
                    />
                    {selectedField.validation?.find(rule => rule.type === "max") && (
                      <Input
                        placeholder="Custom error message"
                        value={
                          selectedField.validation?.find(rule => rule.type === "max")?.message || ""
                        }
                        onChange={(e) => {
                          const message = e.target.value;
                          const currentValidation = selectedField.validation || [];
                          const updatedValidation = currentValidation.map(rule =>
                            rule.type === "max" 
                              ? { ...rule, message: message || `Maximum ${rule.value} characters allowed` }
                              : rule
                          );
                          onUpdateField({ validation: updatedValidation });
                        }}
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      value={
                        selectedField.validation?.find(rule => rule.type === "pattern")?.value || ""
                      }
                      onChange={(e) => {
                        const value = e.target.value;
                        const currentValidation = selectedField.validation || [];
                        const existingPatternRule = currentValidation.find(rule => rule.type === "pattern");
                        ...
                        if (value && value.trim()) {
                          ...
                          const updatedValidation = currentValidation.filter(rule => rule.type !== "pattern");
                          onUpdateField({ validation: updatedValidation });
                        }
                      }}
                    />
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      checked={
                        selectedField.validation?.some(rule => rule.type === "url") || false
                      }
                      onCheckedChange={(checked) => {
                        ...
                        if (checked) {
                          ...
                          if (!currentValidation.some(rule => rule.type === "url")) {
                            onUpdateField({
                              validation: [
                                ...currentValidation,
                                {
                          ...
                          const updatedValidation = currentValidation.filter(rule => rule.type !== "url");
                          onUpdateField({ validation: updatedValidation });
                        }
                      }}
                    />
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      checked={
                        selectedField.validation?.some(rule => rule.type === "pattern" && rule.value === "^[0-9]+$") || false
                      }
                      onCheckedChange={(checked) => {
                        const currentValidation = selectedField.validation || [];
                        ...
                        if (checked) {
                          ...
                          if (!currentValidation.some(rule => rule.type === "pattern" && rule.value === "^[0-9]+$")) {
                            onUpdateField({
                              validation: [
                                ...currentValidation,
                                {
                          ...
                          const updatedValidation = currentValidation.filter(rule => !(rule.type === "pattern" && rule.value === "^[0-9]+$"));
                          onUpdateField({ validation: updatedValidation });
                        }
                      }}
                    />
              ...
              {selectedField.type === "email" && (
                <>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      checked={
                        selectedField.validation?.some(rule => rule.type === "email") || false
                      }
                      onCheckedChange={(checked) => {
                        const currentValidation = selectedField.validation || [];
                        ...
                        if (checked) {
                          ...
                          if (!currentValidation.some(rule => rule.type === "email")) {
                            onUpdateField({
                              validation: [
                                ...currentValidation,
                                {
                          ...
                          const updatedValidation = currentValidation.filter(rule => rule.type !== "email");
                          onUpdateField({ validation: updatedValidation });
                        }
                      }}
                    />
                      ...
                      onChange={(e) => {
                        ...
                        if (value && value !== "") {
                          if (existingMinRule) {
                            ...
                            const updatedValidation = currentValidation.map(rule =>
                              rule.type === "min" 
                                ? { ...rule, value: Number(value), message: `Minimum value is ${value}` }
                                : rule
                            );
                            onUpdateField({ validation: updatedValidation });
                          } else {
                    ...
                    {selectedField.validation?.find(rule => rule.type === "min") && (
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          const updatedValidation = currentValidation.map(rule =>
                            rule.type === "min" 
                              ? { ...rule, message: message || `Minimum value is ${rule.value}` }
                              : rule
                          );
                          onUpdateField({ validation: updatedValidation });
                        }}
                      ...
                      onChange={(e) => {
                        ...
                        if (value && value !== "") {
                          if (existingMaxRule) {
                            ...
                            const updatedValidation = currentValidation.map(rule =>
                              rule.type === "max" 
                                ? { ...rule, value: Number(value), message: `Maximum value is ${value}` }
                                : rule
                            );
                            onUpdateField({ validation: updatedValidation });
                          } else {
                    ...
                    {selectedField.validation?.find(rule => rule.type === "max") && (
                      <Input
                        ...
                        onChange={(e) => {
                          ...
                          const updatedValidation = currentValidation.map(rule =>
                            rule.type === "max" 
                              ? { ...rule, message: message || `Maximum value is ${rule.value}` }
                              : rule
                          );
                          onUpdateField({ validation: updatedValidation });
                        }}
