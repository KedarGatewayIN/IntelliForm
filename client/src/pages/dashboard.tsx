...
export default function Dashboard() {
  ...
  const allTimes = forms
    .flatMap((f) => f.submissions ?? [])
    .map((s) => s.timeTaken)
    .filter((t): t is number => typeof t === "number" && t >= 0);

  const avgSeconds = allTimes.length
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Create, manage, and analyze your intelligent forms
          </p>
        </div>
                  ...
                  <div className="flex items-center space-x-1">
                    <span
                      className={`w-2 h-2 rounded-full ${
                        form.isPublished ? "bg-green-400" : "bg-yellow-400"
                      }`}
                    ></span>
            ...
            <Card className="border-2 border-dashed border-gray-300 hover:border-primary hover:bg-gray-50 transition-all duration-200 cursor-pointer group h-full">
              <CardContent className="p-6 h-full flex flex-col items-center justify-center text-center">
                ...
                <p className="text-gray-500 text-sm">
                  Start building your intelligent form with our drag-and-drop
                  builder
                </p>
              </CardContent>
            </Card>
