...
export default function FormAnalytics() {
  ...
  const [problemResolvePopover, setProblemResolvePopover] = useState<{
    submissionId: string | null;
    problemId: string | null;
  }>({ submissionId: null, problemId: null });
  const [problemResolutionComment, setProblemResolutionComment] =
    useState<string>("");
  ...
  const updateSubmissionProblem = (
    ...
    fetch(`/api/submission/${submission_id}/problem`, {
      ...
      body: JSON.stringify({
        problemId: problem_id,
        resolved,
        resolutionComment: resolved ? comment?.trim() : undefined,
      }),
    })
      .then(async (r) => {
      ...
      .then((updated: ISubmission) => {
        ...
        queryClient.setQueryData<ISubmission[]>(
          ...
          (old) => {
            if (!old) return [];
            return old.map((s) =>
              s.id === submission_id ? { ...s, ...updated } : s,
            );
          },
        );
                          ...
                          <Dialog>
                            ...
                            <DialogContent className="max-w-4xl h-auto max-h-[90vh] flex flex-col overflow-hidden">
                              ...
                              <div className="flex-1 overflow-y-auto pr-2 space-y-6 py-4">
                                <div
                                  className={`rounded-lg p-4 border-l-4 ${(submission.problems?.length ?? 0) === 0 ? "bg-green-50 border-l-green-400" : submission.problems?.some((p) => !p.resolved) ? "bg-red-50 border-l-red-400" : "bg-blue-50 border-l-blue-400"}`}
                                >
                                    ...
                                    <div className="bg-white rounded-md p-4 border border-red-100 space-y-3">
                                      ...
                                      submission.problems.length > 0 ? (
                                            ...
                                            >
                                              ...
                                              <div className="shrink-0">
                                                {p.resolved ? (
                                                  ...
                                                  <Popover
                                                    open={
                                                      problemResolvePopover.submissionId ===
                                                        submission.id &&
                                                      problemResolvePopover.problemId ===
                                                        p.id
                                                    }
                                                    onOpenChange={(open) => {
                                                      if (!open) {
                                                  ...
                                                  >
                                                    ...
                                                    >
                                                      <div className="space-y-4">
                                                        <div>
                                                          <Label
                                                            htmlFor={`resolution-${submission.id}-${p.id}`}
                                                        ...
                                                        <Textarea
                                                          id={`resolution-${submission.id}-${p.id}`}
                                                          value={
                                        ...
                                        <p className="text-sm text-gray-700 leading-relaxed first-letter:capitalize">
                                          No AI issues were flagged for this
                                          submission. The user completed the
                                          form without requiring AI assistance
                                          or encountering any problems.
                                        </p>
                                      )}
                                    </div>
                                  </CardContent>
                                      ...
                                      {submission.aiConversations?.length ? (
                                                    ...
                                                    <div className="space-y-4">
                                                      {conv.messages
                                                        ...
                                                        .map((msg, mIdx) => (
                                                          ...
                                                          >
                                                            <div
                                                              className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${msg.role === "user" ? "bg-blue-500 text-white" : "bg-yellow-400 text-gray-800"}`}
                                                            >
                                                            ...
                                                            >
                                                              <div
                                                                className={`inline-block rounded-2xl px-4 py-2 text-sm shadow-sm ${msg.role === "user" ? "bg-blue-500 text-white rounded-br-md" : "bg-white text-gray-800 border border-gray-200 rounded-bl-md"}`}
                                                              >
                                        ...
                                        <div className="text-center py-8 px-4">
                                          ...
                                          <p className="text-gray-400 text-sm mt-1">
                                            This user completed the form without
                                            AI assistance
                                          </p>
                                        </div>
                                      )}
